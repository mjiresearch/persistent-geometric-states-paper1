...
def load_frozen_distance():
    candidates=[
      Path('data/stationary/frozen/stationary_master_v1.csv'),
    ...
    # De-duplicate identical file/field/value hits and require the frozen master when present.
    uniq=[];seen=set()
    for x in found:
        key=(x['file'],x['field'],x['value'])
        if key not in seen:seen.add(key);uniq.append(x)
    frozen=[x for x in uniq if x['file']=='data/stationary/frozen/stationary_master_v1.csv']
    if frozen:
        vals={x['value'] for x in frozen}
        if len(vals)!=1:raise RuntimeError(f'Frozen NGC0289 distance inconsistent: {frozen}')
        return frozen+ [x for x in uniq if x not in frozen]
    return uniq
...
def main():
    ...
    for comb in combinations(yt,4):
        ys=sorted([q['y'] for q in comb]);ds=[ys[j+1]-ys[j] for j in range(3)];score=max(ds)-min(ds)
    ...
    def linfit(xs,vs):
        ...
        b=sum((x-mx)*(v-mv) for x,v in zip(xs,vs))/sum((x-mx)**2 for x in xs);a=mv-b*mx
        resid=[v-(a+b*x) for x,v in zip(xs,vs)];return a,b,resid
    ax,bx,xres=linfit([q['x'] for q in xmaj],xvals);ay,by,yres=linfit([q['y'] for q in ymaj],yvals)
    pts=[]
    for j,m in enumerate(markers):
        pts.append({'index':j,'pdf_x':m['x'],'pdf_y':m['y'],'radius_kpc_source':ax+bx*m['x'],'sigma_hi_msun_pc2':ay+by*m['y']})
    dxx=[markers[i+1]['x']-markers[i]['x'] for i in range(len(markers)-1)];med=sorted(dxx)[len(dxx)//2];grid_dev=max(abs(v-med) for v in dxx)
    ...
    if not frozen:raise RuntimeError('Frozen NGC0289 distance not found')
    frozen_distance=frozen[0]['value']
    if abs(frozen_distance-20.8)>1e-9:raise RuntimeError(f'Unexpected frozen NGC0289 distance: {frozen_distance}')
    for q in pts:q['radius_kpc_frozen']=q['radius_arcsec']*(frozen_distance*1000.0)/206265.0
    PROFILE.parent.mkdir(parents=True,exist_ok=True)
    fields=['galaxy','stationary_role','source_reference','source_product','source_distance_mpc','frozen_distance_mpc','radius_arcsec','radius_kpc_source','radius_kpc_frozen','sigma_hi_msun_pc2','helium_included','extraction_method','pdf_x','pdf_y']
    with PROFILE.open('w',newline='',encoding='utf-8') as f:
        w=csv.DictWriter(f,fieldnames=fields);w.writeheader()
        for q in pts:
            w.writerow({'galaxy':'NGC0289','stationary_role':'calibration','source_reference':'Lutz_et_al_2018_HIX_II_arXiv1802.04043','source_product':'Figure_A3_panel_b_measured_radial_HI_column_density','source_distance_mpc':source_distance,'frozen_distance_mpc':frozen_distance,'radius_arcsec':f"{q['radius_arcsec']:.8f}",'radius_kpc_source':f"{q['radius_kpc_source']:.8f}",'radius_kpc_frozen':f"{q['radius_kpc_frozen']:.8f}",'sigma_hi_msun_pc2':f"{q['sigma_hi_msun_pc2']:.8f}",'helium_included':'0','extraction_method':'source_native_PDF_vector_marker_centers_linear_native_tick_calibration','pdf_x':f"{q['pdf_x']:.6f}",'pdf_y':f"{q['pdf_y']:.6f}"})
    audit={'status':'HIX2018_NGC0289_NATIVE_VECTOR_HI_PROFILE_RECOVERED','galaxy':'NGC0289','stationary_role':'calibration','n_points':len(pts),
           'source':{'url':final,'package_sha256':sha(payload),'pdf_member':PDF_MEMBER,'pdf_sha256':sha(pdf),'distance_mpc':source_distance},'frozen_distance_mpc':frozen_distance,
           ...
           'ranges':{'radius_kpc_source':[min(q['radius_kpc_source'] for q in pts),max(q['radius_kpc_source'] for q in pts)],'radius_kpc_frozen':[min(q['radius_kpc_frozen'] for q in pts),max(q['radius_kpc_frozen'] for q in pts)],'radius_arcsec':[min(q['radius_arcsec'] for q in pts),max(q['radius_arcsec'] for q in pts)],'sigma_hi_msun_pc2':[min(q['sigma_hi_msun_pc2'] for q in pts),max(q['sigma_hi_msun_pc2'] for q in pts)]},
           'frozen_distance_hits':frozen,'text_evidence':extract_text_context(tex),'boundary':'Public later direct ATCA profile; native vector extraction only. No raster digitization. L_A and C_A remain locked.','output':str(PROFILE)}
    AUDIT.write_text(json.dumps(audit,indent=2)+'\n',encoding='utf-8')
    CAL.write_text('\n'.join([f"status={audit['status']}",f"n_points={len(pts)}",f"x_ticks={xmaj}",f"x_values_kpc={xvals}",f"x_fit={ax:.12g}+({bx:.12g})*x max_resid={audit['x_axis']['max_abs_residual_kpc']:.3g}",f"y_ticks={ymaj}",f"y_values_msun_pc2={yvals}",f"y_fit={ay:.12g}+({by:.12g})*y max_resid={audit['y_axis']['max_abs_residual']:.3g}",f"marker_dx_median={med:.8f} max_dev={grid_dev:.8f}",f"source_distance_mpc={source_distance}",f"frozen_distance_mpc={frozen_distance}",f"frozen_distance_hits={json.dumps(frozen)}",f"ranges={json.dumps(audit['ranges'])}",'quantity=raw_HI helium_included=0','method=measured_from_non_clipped_ATCA_moment0_map_in_elliptical_annuli; geometry from TiRiFiC inclination/PA','No OCR or raster digitization.'])+'\n',encoding='utf-8')
    print(json.dumps({'status':audit['status'],'n_points':len(pts),'ranges':audit['ranges'],'source_distance_mpc':source_distance,'frozen_distance_mpc':frozen_distance,'outputs':[str(PROFILE),str(AUDIT),str(CAL)]},indent=2))
