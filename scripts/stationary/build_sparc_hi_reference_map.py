...
def ref_tokens(s: str) -> list[str]:
    # SPARC Ref can contain comma-separated reference identifiers.
...
def main() -> None:
    ...
    for g in galaxies:
        name = g["Name"]
        role = frozen.get(name)
        if role not in {"calibration", "blind"}:
            continue
        frozen_seen.add(name)
        toks = ref_tokens(g["Ref"])
        if not toks:
            rows.append({
                "galaxy": name, "stationary_role": role, "sparc_ref_id": "",
                "author": "", "bibcode": "", "comment": "",
        ...
        for tok in toks:
            ...
            if rr is None:
                missing_ref_ids.add(tok)
                rr = {"Aut": "", "BibCode": "", "Com": ""}
            rows.append({
                "galaxy": name, "stationary_role": role, "sparc_ref_id": tok,
                "author": rr["Aut"], "bibcode": rr["BibCode"], "comment": rr["Com"],
                "provenance_scope": "SPARC_Table1_column15_HI_Halpha_reference",
            })
    ...
    if missing_ref_ids:
        raise RuntimeError(f"Reference IDs absent from refs table: {sorted(missing_ref_ids)}")

    out = Path("data/stationary/source_reconstruction/sparc_hi_reference_map_v1.csv")
    out.parent.mkdir(parents=True, exist_ok=True)
    fields = ["galaxy", "stationary_role", "sparc_ref_id", "author", "bibcode", "comment", "provenance_scope"]
