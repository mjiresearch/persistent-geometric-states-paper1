...
lines=s.splitlines()
defs=[x.strip() for x in lines if x.lstrip().startswith('/') or ' def' in x]
prog=[x.strip() for x in lines if x.strip() and not x.lstrip().startswith(('%','/'))]
out={'status':'CH06_F11_IDL_STRUCTURE_INSPECTED','defs':defs[:120],'program_lines':prog[:350]}
Path('validation/stationary/ch06_f11_idl_structure_v1.json').write_text(json.dumps(out,indent=2)+'\n')
print(json.dumps({'defs':defs[:80],'program_lines':prog[:120]},indent=2))
