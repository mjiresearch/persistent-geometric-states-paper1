...
for x in a:links += re.findall(r'https?://[^\s"<>]+',x['text'])
hits=[]
try:
...
out={'status':'LGLBS2026_WLM_DATA_ROUTE_AUDITED','attempts':[{k:v for k,v in x.items() if k!='text'} for x in a],'candidate_links':[u for u in links if any(k in u.lower() for k in ['canfar','cadc','26.0020','wlm','profile'])],'canfar_listing_html':a[-1].get('text',''),'source_hits':hits}
