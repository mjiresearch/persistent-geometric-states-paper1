#!/usr/bin/env python3
"""Build fail-closed normalization manifest for all certified stationary H I profiles.

No profile interpolation or persistence evaluation is performed.
"""
...
OUT=Path('data/stationary/source_reconstruction/certified_hi_normalization_manifest_v1.csv')
SUMMARY=Path('validation/stationary/certified_hi_normalization_manifest_v1_summary.json')
...
def main():
 ...
 if len(prov)!=31:raise RuntimeError(f'expected 31 certified, found {len(prov)}')
 ...
 for r in prov:
  ...
  if g in scale:
  ...
  elif 'radius_kpc_frozen' in cols or 'frozen_radius_kpc' in cols:
   ...
   unresolved.append((g,'radius')); radius_method='UNRESOLVED'; radius_factor=''; source_distance=''
  if art==LEROY:
  ...
  rows.append({'galaxy':g,'stationary_role':r['stationary_role'],'acquisition_status':r['effective_acquisition_status'],
   'source_artifact':art,'source_quantity':r['effective_source_quantity'],'source_helium_status':r['effective_helium_status'],
   'frozen_distance_mpc':f'{frozen[g]:.12g}','source_distance_mpc':source_distance,
   'radius_mapping_method':radius_method,'radius_multiplicative_factor_if_source_kpc':'' if radius_factor=='' else f'{radius_factor:.12g}',
   'source_helium_factor_relative_to_raw_HI':source_helium_factor,'helium_mapping_method':helium_method,
   'surface_density_multiplier_to_common_1p33':f'{helium_factor:.12g}','common_surface_density_convention':'Sigma_neutral_1p33 = 1.33 * Sigma_HI_raw',
   'inclination_amplitude_rescale':'0','normalization_ready':'1' if radius_method!='UNRESOLVED' else '0'})
 ...
 result={'status':'CERTIFIED_HI_NORMALIZATION_MANIFEST_V1_COMPLETE','n_certified':len(rows),'n_ready':sum(x['normalization_ready']=='1' for x in rows),
  'role_counts':{'calibration':sum(x['stationary_role']=='calibration' for x in rows),'blind':sum(x['stationary_role']=='blind' for x in rows)},
  'helium_mapping_counts':{m:sum(x['helium_mapping_method']==m for x in rows) for m in sorted({x['helium_mapping_method'] for x in rows})},
  'radius_mapping_counts':{m:sum(x['radius_mapping_method']==m for x in rows) for m in sorted({x['radius_mapping_method'] for x in rows})},
  'policy':'validation/stationary/STATIONARY_HI_COMMON_NORMALIZATION_POLICY_V1.md',
  'boundary':'Normalization metadata only. No profile interpolation, L_A, C_A, tau_A, persistence prediction, or blind outcome evaluated.'}
