...
TARGETS=['DDO154','NGC2403','NGC2841','NGC2976','NGC3198']
...
def main():
 ...
 if any(roles[g]!='calibration' for g in TARGETS):raise RuntimeError('target role changed')
 ...
 # Find deluxetable/table blocks and identify global H I properties by S_HI/M_HI headings.
 blocks=[]
 ...
 # Collect exact candidate rows for each target from full source and from H I blocks.
 out={}
 for g in TARGETS:
  ...
  # Geometry row: target row with >=10 fields containing RA/Dec/dist; beam row explicitly NA.
 ...
 # Wang fixed-width parsing from VizieR ReadMe column layout used in prior certified audit.
 wt=fetch(WANG).decode('utf-8','replace'); wang=[]
 for line in wt.splitlines():
  if not line.strip():continue
  for g in TARGETS:
   if compact(g) not in compact(line[:20]):continue
   # Persist raw row here; final strict parser can use the already-audited Wang layout.
   wang.append({'galaxy':g,'raw_line':line})
 result={'status':'THINGS_CALIBRATION_VALIDATION_METADATA_EXTRACTED','targets':TARGETS,'walter_hi_block_count':len(hi_blocks),'walter_hi_block_heads':[b[:2500] for b in hi_blocks],
  'targets_raw':out,'wang_target_rows':wang,
  'boundary':'Published source-domain metadata only; no science pixels, rotation residuals, persistence parameters, or blind outcomes.'}
 OUT.parent.mkdir(parents=True,exist_ok=True);OUT.write_text(json.dumps(result,indent=2)+'\n');print(json.dumps({'status':result['status'],'hi_block_count':len(hi_blocks),'targets':out,'wang_rows':wang},indent=2))
