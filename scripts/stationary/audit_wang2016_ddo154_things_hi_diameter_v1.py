...
VizieR catalogue and records the DDO154 THINGS row.  Source-domain metadata only;
no rotation residuals or persistence quantities are evaluated.
"""
...
def main():
 ...
 matches=[r for r in rows if compact(r['Name'])=='ddo154']
 if len(matches)!=1:raise RuntimeError(f'expected one DDO154 row; got {matches}')
 r=matches[0]
 if compact(r['Sample'])!='things':raise RuntimeError(f'DDO154 row is not THINGS: {r}')
 result={'status':'WANG2016_DDO154_THINGS_HI_DIAMETER_AUDITED','catalog':'J/MNRAS/460/2143/table2','retrieval_url':u,'final_url':final,'n_catalog_rows_parsed':len(rows),'row':r,
  ...
  'provenance_note':'For the THINGS sample Wang et al. use Walter et al. 2008 interferometric data; this is a matched independent published size measurement for the same THINGS product family and raw-HI threshold.',
