...
BD_COLUMNS = [
    "ID", "ID_MPA_JHU", "ra", "dec", "z", "D_A_Mpc", "D25_arcsec", "b_a",
    "R50_arcsec", "R90_arcsec", "r_exp_arcsec", "mag_u", "mag_g", "mag_r",
    "mag_i", "mag_z", "NUV_r", "Delta_g_i", "logMstar", "log_mu_st",
    "logMHI", "date",
]
...
def parse_bluedisk_overview(text: str) -> list[dict[str, str]]:
    ...
    if len(lines) < 51:
        raise RuntimeError(f"Expected Bluedisk header + 50 rows, got {len(lines)} nonblank lines")
    rows = []
    for lineno, line in enumerate(lines[1:], start=2):
        vals = line.split()
        if len(vals) != len(BD_COLUMNS):
            raise RuntimeError(
                f"Bluedisk overview line {lineno}: expected {len(BD_COLUMNS)} fields, "
                f"got {len(vals)}: {line!r}"
            )
        rows.append(dict(zip(BD_COLUMNS, vals)))
    if len(rows) != 50:
...
def parse_asu(text: str) -> list[dict[str, str]]:
    ...
    if header is None:
        raise RuntimeError("SPARC VizieR header not found")
    out = []
...
def main():
    ...
    for b in bd:
        ...
        ranked = sorted(
            (sep_arcsec(bra, bdec, float(s["_RA"]), float(s["_DE"])), s)
            for s in sp
        )
    ...
    fields = [
        "bluedisk_id", "bluedisk_ra_deg", "bluedisk_dec_deg", "bluedisk_DA_mpc",
        "sparc_name", "stationary_role", "separation_arcsec",
        "second_nearest_separation_arcsec", "unambiguous_15arcsec_match",
    ]
