...
V3 preserves edge-only source blanks as missing and consumes the already-certified
NGC5907 m=0 product, mapping only sub-resolution near-zero negative radii to zero
under STATIONARY_HI_NUMERICAL_TOLERANCE_POLICY_V1.
...
OUT_TAB=Path('data/stationary/source_reconstruction/certified_hi_common_tabulated_v3.csv')
OUT_ANA=Path('data/stationary/source_reconstruction/certified_hi_common_analytic_v3.csv')
SUMMARY=Path('validation/stationary/certified_hi_common_normalized_v3_summary.json')
NGC5907_QC=Path('validation/stationary/sa87_ngc5907_heroes2015_native_hi_recovery_v1.json')
...
def qc_tolerance_kpc():
 q=json.loads(NGC5907_QC.read_text())
 vals=[]
 def walk(x):
  if isinstance(x,dict):
   for k,v in x.items():
    if k=='max_tick_residual_kpc' and isinstance(v,(int,float)):vals.append(abs(float(v)))
    walk(v)
  elif isinstance(x,list):
   for v in x:walk(v)
 walk(q)
 if not vals:raise RuntimeError('NGC5907 QC no max_tick_residual_kpc found')
 return max(vals)
...
def main():
 ...
 tab=[]; missing_excluded=defaultdict(int); tolerance_adjusted=[]
 ...
 for g,rr in sorted(by_leroy.items()):
  ...
  scale=float(mm[g]['radius_multiplicative_factor_if_source_kpc']); out_idx=0
  for i,r in enumerate(rr):
   sigma=sig[i]
   if sigma is None or not math.isfinite(sigma):
    missing_excluded[g]+=1; continue
 ...
 scale=float(mm['NGC5907']['radius_multiplicative_factor_if_source_kpc']); tol=qc_tolerance_kpc()
 for i,r in enumerate(n5907):
  ...
  if rs < 0:
   if abs(rs) <= tol:
    adjusted=0.0; tolerance_adjusted.append({'galaxy':'NGC5907','sample_index':idx,'source_radius_kpc':rs,'tolerance_kpc':tol,'normalized_source_radius_kpc':0.0})
   else:raise RuntimeError(f'NGC5907 negative radius exceeds source calibration tolerance: {rs} < -{tol}')
  emit('NGC5907',idx,rs,'kpc_source',adjusted*scale,sigma,
       source_note='certified deterministic m0 mean; sub-resolution near-zero negative radius mapped to zero only when within source-axis calibration QC tolerance')
 if len(tolerance_adjusted)!=1:raise RuntimeError(f'unexpected NGC5907 tolerance adjustments: {tolerance_adjusted}')

 tab_gals=sorted({r['galaxy'] for r in tab}); expected_tab=sorted(g for g,m in mm.items() if m['acquisition_status']=='raw_source_profile_ingested')
 ...
 for r in read_csv(ART['jadhav']):
  g=r['galaxy'];m=mm[g]; mult=float(m['surface_density_multiplier_to_common_1p33']); scale=float(m['radius_multiplicative_factor_if_source_kpc']); has2=fval(r['sigma02_msun_pc2']) is not None
  ...
  ana.append({'galaxy':g,'stationary_role':m['stationary_role'],'source_artifact':m['source_artifact'],'profile_family':'offcentered_gaussian_sum',
   ...
   'surface_density_multiplier_to_common_1p33':m['surface_density_multiplier_to_common_1p33'],'radius_mapping_method':m['radius_mapping_method'],'inclination_amplitude_rescale':'0'})
 ana.sort(key=lambda x:x['galaxy']); expected_ana=sorted(g for g,m in mm.items() if m['acquisition_status']=='analytic_profile_recovered')
 ...
 result={'status':'CERTIFIED_HI_COMMON_NORMALIZED_V3_BUILT','n_certified_galaxies':len(tab_gals)+len(ana),'n_tabulated_galaxies':len(tab_gals),'n_analytic_galaxies':len(ana),
  ...
  'n_numerical_tolerance_adjustments':len(tolerance_adjusted),'numerical_tolerance_adjustments':tolerance_adjusted,
  ...
  'missing_value_policy':'validation/stationary/STATIONARY_HI_MISSING_VALUE_POLICY_V1.md','numerical_tolerance_policy':'validation/stationary/STATIONARY_HI_NUMERICAL_TOLERANCE_POLICY_V1.md',
