...
def main():
 ...
 for g in TARGETS:
  ...
  # The ECSV's Dist column is published to one decimal place. Do not demand
  # sub-rounding agreement between its rounded Dist and radi_kpc, which may
  # have been generated from a more precise internal distance. Instead recover
  # the distance implied independently by each (arcsec,kpc) pair and require
  # the median to lie within half of the displayed 0.1-Mpc rounding unit.
  implied_dist=k*1000/(a*PC_PER_ARCSEC_PER_MPC)
  implied_dist_median=float(np.nanmedian(implied_dist)) if len(implied_dist) else math.nan
  implied_dist_span=float(np.nanmax(implied_dist)-np.nanmin(implied_dist)) if len(implied_dist) else math.inf
  displayed_distance_half_unit_mpc=0.05
  source_distance_abs_difference=abs(implied_dist_median-source_dist) if np.isfinite(implied_dist_median) else math.inf
  ...
  checks={
   ...
   'source_distance_implied_by_radii_consistent_with_displayed_distance_precision':bool(source_distance_abs_difference<=displayed_distance_half_unit_mpc),
   'implied_distance_constant_across_profile':bool(implied_dist_span<=1e-8),
  ...
  with outcsv.open('w',newline='',encoding='utf-8') as f:
   ...
   for av,kv,fkv,sv in zip(a,k,frozen_kpc,s):
    w.writerow({'galaxy':g,'stationary_role':'calibration','source':'Wang et al. 2025 FEASTS HIprof_wang25.ecsv','radius_arcsec':f'{av:.15g}','radius_kpc_source_distance':f'{kv:.15g}','radius_kpc_frozen_distance':f'{fkv:.15g}','sigma_hi_raw_msun_pc2':f'{sv:.15g}','source_distance_mpc':f'{source_dist:.15g}','source_distance_implied_by_radii_mpc':f'{implied_dist_median:.15g}','frozen_distance_mpc':f'{fd:.15g}','helium_included':'0'})
  results.append({
   ...
   'source_profile_file':str(FEASTS),'normalized_profile_csv':str(outcsv),'source_distance_mpc_displayed':source_dist,
   'source_distance_mpc_implied_by_radii':implied_dist_median,'source_distance_implied_span_mpc':implied_dist_span,
   'displayed_distance_half_rounding_unit_mpc':displayed_distance_half_unit_mpc,'source_distance_abs_difference_mpc':source_distance_abs_difference,
   'frozen_distance_mpc':fd,'n_array_slots':len(aa),'n_finite_profile_points':len(a),
   'radius_arcsec_min':float(a.min()) if len(a) else None,'radius_arcsec_max':float(a.max()) if len(a) else None,
 ...
 payload={'status':'FEASTS2025_NEW_CALIBRATION_PROFILE_QC_COMPLETE','source_release':'Wang et al. 2025 FEASTS / HIprof_wang25.ecsv','source_doi':'10.3847/1538-4357/ada95a','source_arxiv':'2501.01289','helium_convention':'raw atomic H I; no helium included','distance_precision_rule':'ECSV Dist is displayed to 0.1 Mpc; (radius_arcsec,radius_kpc) implied distance must agree within half the displayed unit, 0.05 Mpc. This rule replaces an erroneously over-tight relative check from the first diagnostic run.','targets':list(TARGETS),'n_pass':sum(x['passes_all_source_qc'] for x in results),'results':results,'promotion_boundary':'This audit does not promote profiles. Promotion requires a separate overlay update after reviewing these source-only QC results.','science_boundary':'No rotation velocities, residuals, persistence parameters, L_A, C_A, tau_A, or blind outcomes read.'}
