...
This script uses only frozen observational inputs. It does NOT fit or evaluate
L_A, C_A, tau_A, or any persistence-model prediction.

Important distinction
---------------------
The SPARC 2016 mass-model table provides inclination-corrected stellar surface
brightnesses but does not provide the radial HI surface-density profile. Vgas
is a Newtonian velocity contribution, not a surface density. Therefore this
script does not infer Sigma_gas by treating Vgas as a density proxy.

Outputs
-------
data/stationary/source_reconstruction/stationary_source_basis_v1.csv
...
# Hua et al. (2025, A&A 703, A223) report HI surface-density profiles for
# 169/175 SPARC galaxies and identify these six as lacking radial HI profiles
# in the underlying rotation-curve references.
...
def main() -> None:
    ...
    basis_fields = [
        ...
        "v_err_kms",
        "v_gas_kms",
        "v_gas2_signed_km2_s2",
        "v_disk_ml1_kms",
        "v_disk2_per_unit_upsilon_km2_s2",
        "v_bulge_ml1_kms",
        "v_bulge2_per_unit_upsilon_km2_s2",
        "sb_disk_lsun_pc2",
        "sb_bulge_lsun_pc2",
        "sigma_disk_per_unit_upsilon_msun_pc2",
        "sigma_bulge_per_unit_upsilon_msun_pc2",
        "sigma_gas_msun_pc2",
        "sigma_gas_status",
    ...
    for r in master:
        ...
        basis_rows.append(
            {
                "galaxy": g,
                "sample_role": split_by_galaxy[g]["sample_role"],
                ...
                # Numerically equal to SB at M/L=1, but named explicitly as a
                # mass-density basis. Actual Sigma_* is Upsilon times this basis.
    ...
    # One row per galaxy recording whether an external radial HI profile is
    # reported to exist. Acquisition is a separate, auditable step.
    by_galaxy_master: dict[str, list[dict[str, str]]] = {}
    for r in master:
        ...
        availability_rows.append(
            {
                ...
                "primary_source_reconstruction_status": (
                    "await_external_HI_profile_ingestion"
                    if reported
                ...
                "note": (
                    ...
                    if not reported
                    else "Radial HI profile reported available in the literature compilation; exact source/provenance must be ingested before use."
                ),
            }
        )
    ...
    summary = {
        ...
        "gas_surface_density_rule": "Do not substitute Vgas for Sigma_gas. Ingest an independently sourced radial HI profile or apply a separately validated predeclared reconstruction for missing-profile systems.",
        "source_current_velocity_rule": "Use the self-consistent model velocity V(R) in the stationary current; Vobs remains target data and is not used as source current.",
