...
def correlation_block(d: pd.DataFrame, history_cols: list[str], target_cols: list[str]) -> dict:
    ...
    for h in history_cols:
        ...
        for t in target_cols:
            out[h][t] = corr_stats(r, d[h].to_numpy(float), d[t].to_numpy(float))
    return out
...
def main() -> None:
    ...
    vbar, vhalo_model, vtotal_model, vobs = [], [], [], []
    ...
    support_masks = {
        'all_radial_bins': np.ones(len(radial), dtype=bool),
        'supported_bins_stars_ge100_cells_ge5': (radial['stars_weight'] >= 100) & (radial['azimuth_vertical_cells'] >= 5),
        'high_support_bins_stars_ge300_cells_ge9': (radial['stars_weight'] >= 300) & (radial['azimuth_vertical_cells'] >= 9),
    }
    robustness = {}
    for label, mask in support_masks.items():
        d = radial.loc[mask].copy()
        robustness[label] = {
            'n_bins': int(len(d)),
            'R_range_kpc': [float(d['R_kpc'].min()), float(d['R_kpc'].max())] if len(d) else None,
            'correlations': correlation_block(d, history_cols, target_cols),
        }
    ...
    for rc in [6.0, 8.0, 10.0, 12.0]:
        ...
        checkpoints[str(rc)] = {
            ...
            'stars_weight': int(q['stars_weight']),
            'azimuth_vertical_cells': int(q['azimuth_vertical_cells']),
        }
    ...
    report = {
        ...
        'support_robustness_screens': robustness,
        'guardrail': (
            'This is a screening comparison, not a persistence detection. Raw radial correlations can be generated by '
            'shared radial gradients. The supported-bin and high-support screens are primary robustness checks; '
            'detrended and first-difference results should be weighted more strongly than all-bin raw correlations. '
            'The next decisive test requires a spatial baryonic force residual and independently reconstructed source history.'
        )
    }
