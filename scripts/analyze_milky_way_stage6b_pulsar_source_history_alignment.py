...
def weighted_field(sx,sy,sz,vals,px,py,pz,sigma):
 d2=(sx-px)**2+(sy-py)**2+(sz-pz)**2;m=d2<=(3.0*sigma)**2
 if m.sum()==0:return {k:np.nan for k in vals}|{'source_weight':0.0,'source_neff':0.0,'nearest_source_kpc':float(np.sqrt(np.min(d2)))}
 w=np.exp(-0.5*d2[m]/sigma**2);sw=float(w.sum());neff=float(sw*sw/np.sum(w*w));out={k:float(np.sum(w*v[m])/sw) for k,v in vals.items()};out.update({'source_weight':sw,'source_neff':neff,'nearest_source_kpc':float(np.sqrt(np.min(d2)))});return out
def build_fields(pul,src,sigma,y_sign):
 R=src.R_gal.to_numpy(float);ph=np.deg2rad(src.phi_deg.to_numpy(float));sx=R*np.cos(ph);sy=R*np.sin(ph);sz=src.Z_gal.to_numpy(float);delta=src.deltaR_present_birth_kpc.to_numpy(float);outward=src.outward_present_gt1.to_numpy(float);inward=src.inward_present_gt1.to_numpy(float)
 ...
 n=len(pul);na=len(OFFSETS);data={k:np.full((na,n),np.nan) for k in list(vals)+['source_weight','source_neff','nearest_source_kpc','log_source_weight']};x0=-pul.x_pc.to_numpy(float)/1000.0;y0=y_sign*pul.y_pc.to_numpy(float)/1000.0;z0=pul.z_pc.to_numpy(float)/1000.0;Rp=np.hypot(x0,y0);phi0=np.arctan2(y0,x0)
...
def rotation_test(pul,field,target,sample_mask,predictors):
 ...
 obs=rhos[0].copy();null=np.abs(rhos[1:]);tests={};maxnull=np.nanmax(null,axis=1)
 for j,p in enumerate(predictors):
  if not np.isfinite(obs[j]):tests[p]={'rho_actual':None};continue
  good=np.isfinite(null[:,j]);pperm=float((1+np.sum(null[good,j]>=abs(obs[j])))/(1+good.sum()));gm=np.isfinite(maxnull);pmax=float((1+np.sum(maxnull[gm]>=abs(obs[j])))/(1+gm.sum()));tests[p]={'rho_actual':float(obs[j]),'n_actual':int(ns[0,j]),'p_azimuth_rotation':pperm,'p_maxT_predictor_family':pmax,'null_abs_rho_median':float(np.nanmedian(null[:,j])),'null_abs_rho_95pct':float(np.nanpercentile(null[:,j],95))}
 return tests,rhos
def main():
 ...
 src=src.dropna(subset=['R_gal','Z_gal','phi_deg','Rbirth_proxy_kpc','deltaR_present_birth_kpc','Age','FeH','aFe']).copy();src=src[src.R_gal.between(4.0,11.0)&src.Z_gal.abs().le(3.0)].copy();runs={};flat=[]
 for ysign in [1.0,-1.0]:
  hand='Ysame' if ysign>0 else 'Yreflected';runs[hand]={}
  for sigma in SIGMAS:
   ...
   for sname,fn in SAMPLES.items():
    mask=fn(pul)&np.isfinite(pul.resid_baryon_1e10.to_numpy(float));ht,hr=rotation_test(pul,field,'resid_baryon_1e10',mask,HISTORY);rt,rr=rotation_test(pul,field,'resid_baryon_1e10',mask,REFERENCES);zt,zr=rotation_test(pul,field,'zresid_baryon',mask,HISTORY);runs[hand][key]['samples'][sname]={'n_pulsars':int(mask.sum()),'history_vs_baryonic_residual':ht,'reference_fields_vs_baryonic_residual':rt,'history_vs_standardized_residual':zt}
 ...
 pd.DataFrame(flat).to_csv(OUT/'stage6b_history_test_matrix.csv',index=False);f=pd.DataFrame(flat);f=f[f.target=='resid_baryon_1e10'].copy();cons={}
 for p,g in f.groupby('predictor'):cons[p]={'n_tests':int(len(g)),'n_maxT_lt_0p05':int((g.p_maxT_predictor_family<.05).sum()),'n_rotation_lt_0p05':int((g.p_azimuth_rotation<.05).sum()),'positive_rho_fraction':float((g.rho_actual>0).mean()),'negative_rho_fraction':float((g.rho_actual<0).mean()),'min_p_maxT':float(g.p_maxT_predictor_family.min()),'rho_range':[float(g.rho_actual.min()),float(g.rho_actual.max())]}
