...
def robust_geometry(image,threshold_fraction=.03):
    ...
    cxy=float((z*dx*dy).sum()/wt)
    vals,vecs=np.linalg.eigh([[float((z*dx*dx).sum()/wt),cxy],[cxy,float((z*dy*dy).sum()/wt)]])
...
def extract(image,h,inc_deg,dist_mpc,central_beams=2.,annulus_beams=.5):
    ...
    k=dist_mpc*1e3/206265.;rell=rell_as*k;mask=central_beams*beam*k;dr=annulus_beams*beam*k
    # Match the previously frozen map reduction: zero-valued blank/background pixels are excluded.
    ...
    for lo,hi in zip(edges[:-1],edges[1:]):
        ...
        rows.append({"r_kpc":.5*(lo+hi),"Sigma_HI_Msun_pc2":float(np.nanmedian(s)),"Sigma_HI_p16":float(np.nanpercentile(s,16)),"Sigma_HI_p84":float(np.nanpercentile(s,84)),"n_pix_mom0":int(s.size),"beam_smeared_mask":bool(.5*(lo+hi)<mask)})
    return pd.DataFrame(rows),{"center_x_pix":xc,"center_y_pix":yc,"pa_pixel_deg":pa,"inclination_deg":inc_deg,"distance_mpc":dist_mpc,"beam_arcsec":beam,"central_mask_kpc":mask,"annulus_width_kpc":dr,"BUNIT_header":str(h.get("BUNIT","")),"unit_qc":"coldens_values_physical_NHI_header_BUNIT_stale","background_policy":"strictly_positive_pixels_only"}
...
def main():
    ...
    for gal in NEW_TARGETS:
        ...
        try:
            ...
            qc.append({**base,"qc_status":"excluded_from_thin_annulus_extraction","qc_reason":str(e)});print(gal,"QC EXCLUSION",e)
    ...
    if len(profiles):
        clean=profiles[(~profiles.beam_smeared_mask)&np.isfinite(profiles.Sigma_HI_Msun_pc2)];print("clean profile bins by galaxy:");print(clean.groupby("name").size().to_string());print("clean radial ranges:");print(clean.groupby("name").r_kpc.agg(['min','max']).to_string())
